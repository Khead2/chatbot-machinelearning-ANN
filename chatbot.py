...
while True:  # Loop tanpa henti
    try:
        x = input("You: ")  # Input dari pengguna
        if x.lower() in ['exit', 'quit']:  # Perintah untuk keluar
            print("Chatbot: Sampai jumpa!")
            break
        response = chatbot_response(x)  # Memanggil fungsi untuk mendapatkan respons

        # Pastikan response adalah string sebelum menggunakan strip
        if isinstance(response, str):
            response = response.strip()

        print(f"Chatbot: {response}")  # Menampilkan respons chatbot
